...
<!--       <div class="row">
        <div class="col">
          <form class="input-group">
            <input placeholder="Escribe para empezar a buscar" class="form-control" type="text" name="buscar" id="buscar">
          </form>
      ...
      <div class="row">
        <div class="col">
          <form class="input-group" method="POST">
            <input id="publicar" name="publicar" class="form-control" type="text" placeholder="Crea una publicación" aria-label="Post">
          </form>
        </div>             
      </div>
    ...
    <script type="text/javascript">
      $(function(){
        ...
        $(".btn_comentarios").click(function(){
          ...
          window.location.href = "comentarios.php?id_comentario="+ id_comentario;
        });

        $(".down").click(function() {
          let publicacionId = $(this).attr("id");

          $.ajax({
              url: 'reaccion_down.php',
              type: 'POST',
          ...
          if($("#buscar").val()){
            ...
            $.ajax({
              ...
              success: function(response) {
                ...
                rows.forEach(row => {
                 template += `<tr><td> ${row.autor}</td><td> ${row.titulo}</td><td> ${row.descripcion}</td><td> ${row.up}</td><td> ${row.down}</td></tr>`
                });
        ...
        $("#publicar").click(function(){
          window.location.href = "form_publicar.php";
        });
